...
public class ItemTransmutationRod extends ItemBase
{
	 ...
	 * Used to search for items to transmutate
	 */
	@Nonnull
	@Override
			...
			EnumFacing side, float par8, float par9, float par10)
	{
		...
		{
			List<EntityItem> items = world.getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(pos.add(-2, -2, -2), pos.add(2, 2, 2)));
			for (EntityItem item : items)
			{
				if (item.getEntityItem().getItem() == Items.GLASS_BOTTLE)
				{
					EntityItem vialEmpty = new EntityItem(world, item.posX, item.posY, item.posZ, new ItemStack(ModItems.essence_vial_empty));
					if (!world.isRemote)
					{
