...
public class SpellFireball extends Spell {
	...
	public SpellFireball() {
		super("fireball", new Essence[] {}, ModItems.SPELL_FIREBALL, 0, 5, 1, 5, false, true);
	}
	...
	public boolean onCast(ItemStack wand, EntityPlayer player, World world, BlockPos pos, EnumHand hand,
		...
		if (WandHelper.canUseEssence(wand, cost, pair.getKey().getCoreType())) {
			...
			for (int i = 0; i < 25; i++) {
				...
				player.getEntityWorld().spawnParticle(EnumParticleTypes.FALLING_DUST, x, y, z, 0.0D, 0.0D, 0.0D, pars);
			}

			WandHelper.useEssence(wand, cost, pair.getKey().getCoreType());
			world.playSound(null, player.getPosition(), SoundEvents.ENTITY_ENDERDRAGON_FIREBALL_EPLD,
					SoundCategory.BLOCKS, 1, 1);
			player.getCooldownTracker().setCooldown(wand.getItem(), cooldown);
			EntityLargeFireball bigBall = new EntityLargeFireball(world, player, player.getLookVec().xCoord,
					player.getLookVec().yCoord, player.getLookVec().zCoord);
			...
			if (!world.isRemote)
				world.spawnEntityInWorld(bigBall);
			player.getCooldownTracker().setCooldown(wand.getItem(), cooldown);
			return true;
